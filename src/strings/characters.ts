...
/**
 * Returns the character size (in unicode code points) of the
 * character at the leading or trailing edge of the provided string.
 */
export function getEdgeCharacterSize(
  str: string,
  edge: "leading" | "trailing",
  isRTL?: boolean
) {
  if (edge === "leading") {
    return getCharacterDistance(str, isRTL);
  }

  // For Thai script, deleting N character(s) backward should delete
  // N code point(s) instead of an entire grapheme cluster.
  const lastChar = str[str.length - 1];
  if (lastChar && isThaiScript(lastChar)) {
    return 1;
  }

  let characterEnd = getCharacterDistance(str, false);
  let remaining = str.slice(characterEnd);

  while (remaining.length) {
    characterEnd = getCharacterDistance(remaining, false);
    remaining = remaining.slice(characterEnd);
  }
...
/**
 * Determines whether the character is a part of the Thai script
 */
export function isThaiScript(char: string) {
  return char.match(/[\u0E00-\u0E7F]/);
}

// Taken from https://github.com/ianstormtaylor/slate/blob/main/packages/slate/src/utils/string.ts
...
export function* codepointsIteratorRTL(str: string) {
