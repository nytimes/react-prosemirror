 ...
 */ /* eslint-disable @typescript-eslint/no-explicit-any */ /* eslint-disable @typescript-eslint/no-non-null-assertion */ import { expect } from "@jest/globals";
import { act, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { EditorState, TextSelection } from "prosemirror-state";
import { a, blockquote, br, doc, em, img as img_, p, pre, strong } from "prosemirror-test-builder";
...
describe("DOM change", ()=>{
    ...
    it("handles a deep split of nodes", ()=>{
        ...
        flush(view);
        expect(view.state.doc).toEqualNode(doc(blockquote(p("abx")), blockquote(p("cd"))));
        expect(view.state.selection.anchor).toBe(9);
    });
    it("can delete the third instance of a character", ()=>{
        const { view  } = tempEditor({
            doc: doc(p("foo xxx<a> bar"))
        });
    ...
    it("can delete text in markup", ()=>{
        ...
        flush(view);
        expect(view.state.doc).toEqualNode(doc(p("a", em("b", img, strong("c")), "e")));
    });
    it("recognizes typing inside markup", ()=>{
        ...
        flush(view);
        expect(view.state.doc).toEqualNode(doc(p("a", em("b", img, strong("cdxy")), "e")));
    });
    ...
    it("doesn't confuse delete with backspace for multi-character deletions", async ()=>{
        ...
        flush(view);
        expect(steps).toHaveLength(1);
        expect(steps[0].from).toBe(8);
        expect(steps[0].to).toBe(11);
    });
