...
describe("EditorView draw", ()=>{
    ...
    it("correctly wraps blokc nodes with marks", async ()=>{
        const testSchema = new _prosemirrorModel.Schema({
            nodes: _prosemirrorTestBuilder.schema.spec.nodes.update("image", {
                ..._prosemirrorTestBuilder.schema.spec.nodes.get("image"),
                inline: false,
                group: "block"
            }),
            marks: _prosemirrorTestBuilder.schema.spec.marks.addToEnd("difficulty", {
                attrs: {
                    level: {
                        default: "beginner"
                    }
        ...
        const { doc , image , difficulty  } = (0, _prosemirrorTestBuilder.builders)(testSchema);
        const { view  } = (0, _editorViewTestHelpersJs.tempEditor)({
