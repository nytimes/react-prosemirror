...
let ViewDesc = class ViewDesc {
    ...
    localPosFromDOM(dom, offset, bias) {
        ...
        if (dom == this.dom && this.contentDOM) {
            atEnd = offset > (0, _selectionToDOMJs.domIndex)(this.contentDOM);
    ...
    domFromPos(pos, side) {
        ...
        if (side <= 0) {
            ...
            return {
                ...
                offset: prev ? (0, _selectionToDOMJs.domIndex)(prev.dom) + 1 : 0
            };
        } else {
            let next, enter = true;
            for(;; i++, enter = false){
            ...
            return {
                node: this.contentDOM,
                offset: next ? (0, _selectionToDOMJs.domIndex)(next.dom) : this.contentDOM.childNodes.length
            };
        }
    }
            ...
            if (fromOffset == -1 && from <= end) {
                ...
                for(let j = i; j > 0; j--){
                    ...
                    if (prev.size && prev.dom.parentNode == this.contentDOM && !prev.emptyChildAt(1)) {
                        fromOffset = (0, _selectionToDOMJs.domIndex)(prev.dom) + 1;
                        break;
                    }
                    from -= prev.size;
                }
            ...
            if (fromOffset > -1 && (end > to || i == this.children.length - 1)) {
                ...
                for(let j = i + 1; j < this.children.length; j++){
                    ...
                    if (next.size && next.dom.parentNode == this.contentDOM && !next.emptyChildAt(-1)) {
                        toOffset = (0, _selectionToDOMJs.domIndex)(next.dom);
                        break;
                    }
                    to += next.size;
                }
    ...
    setSelection(anchor, head, root) {
                    ...
                    for(let scan = node, after; scan; scan = scan.parentNode){
                        if (after = scan.nextSibling) {
                            if (after.nodeName == "BR") anchorDOM = headDOM = {
                                ...
                                offset: (0, _selectionToDOMJs.domIndex)(after) + 1
                            };
                            break;
                        }
        ...
        if (_browserJs.browser.gecko && domSel.focusNode && domSel.focusNode != headDOM.node && domSel.focusNode.nodeType == 1) {
            const after = domSel.focusNode.childNodes[domSel.focusOffset];
            if (after && after.contentEditable == "false") force = true;
        }
        if (!(force || brKludge && _browserJs.browser.safari) && (0, _selectionToDOMJs.isEquivalentPosition)(anchorDOM.node, anchorDOM.offset, domSel.anchorNode, domSel.anchorOffset) && (0, _selectionToDOMJs.isEquivalentPosition)(headDOM.node, headDOM.offset, domSel.focusNode, domSel.focusOffset)) return;
